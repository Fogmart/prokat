...
class ModCfilteringOptions
{
	...
	public function getRelativePriceRanges()
	{
		...
		if(empty($ranges)){
			...
			$ranges=$this->caclFinalPrice($ranges);
			if(!empty($ranges->min_value))$ranges->min_value=floor($ranges->min_value);
			if(!empty($ranges->max_value))$ranges->max_value=ceil($ranges->max_value);
	...
	public function caclFinalPrice($ranges)
	{
		...
		if((int)$vendor_currency['vendor_currency'] !=$shop_currency_id){
			...
			if(!empty($ranges->min_value))$ranges->min_value=$vmCurrencyHelper->convertCurrencyTo($shop_currency_id, $ranges->min_value,$shop=false);
			if(!empty($ranges->max_value))$ranges->max_value=$vmCurrencyHelper->convertCurrencyTo($shop_currency_id, $ranges->max_value,$shop=false);
		}
	...
	public function buildPriceRangeQuery($query,$part=false)
	{
		...
		}else $query->where('p.product_parent_id=0');
	...
	public static function getDisplayControl($flt_sfx, $selected_flt,$params)
	{
		...
		if($is_published){
			...
			if($disp){
				...
				elseif($flt_sfx=='manuf_flt')$disp_with_fltrs=$params->get('manuf_flt_disp_after','1');
