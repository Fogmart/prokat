...
JHtml::stylesheet('//netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.css', array(), false);
...
if($product_orderable || $notorderable_enabled) {
	...
	$form_deletefavorite = '<form style="text-align: center; margin:0px" action="';
	...
	if ($result > 0 ){
			...
			else
			{
				...
				else 
				{
					...
					$addtofavorites .= '<div class="btn addtofav_aws_icon_active"';
					...
					elseif($tmpl_favlist_style == "heart") $addtofavorites .= '<i class="zmdi zmdi-favorite "></i>';
	...
	if ($result == 0){
		if($product_id == $favorite_id && $mode == "fav_add") {
			...
			if ($guest_enabled || !$user->guest){
					$addtofavorites .= '
					<form style="text-align: center; margin:0px" method="post" action="'.$url_fav.'" name="addtofavorites" id="addtofavorites_'.$product_id.'">';
					...
					if ($tmpl_favlist_style == "button") $addtofavorites .= '"btn modns button art-button art-button"';
					else $addtofavorites .= '"btn addtofav_aws_icon"';
					...
					}elseif($tmpl_favlist_style == "heart") $addtofavorites .= '<i class="zmdi zmdi-favorite-outline "></i>';
					else $addtofavorites .= '<i class="fa fa-'.$tmpl_favlist_style.'"></i>';
