...
<form method="get" action="<?php echo JRoute::_('index.php?option=com_customfilters&view=products&Itemid='.$Itemid)?>" class="cf_form<?php echo $moduleclass_sfx;?>" id="cf_form_<?php echo $module->id?>">
	...
	foreach($filters_html_array as $key=>$flt_html){
		$class = "";
		$class_key = explode('_', $key);
		if($class_key[0] && $class_key[1]){
			if($class_key[1])
				$class = implode('_', array($class_key[0], $class_key[1]));
			else
				$class = $class_key[0];
		}
	?> 
	<div class="cf_flt_wrapper <?php echo $class; ?>  cf_flt_wrapper_id_<?php echo $module->id?> cf_flt_wrapper_<?php echo $direction ?>" id="cf_flt_wrapper_<?php echo $key?>" role="presentation">
