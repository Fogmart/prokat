...
<?php elseif ($params->get('dropdown', 1) && $params->get('dropdownimage', 0)) : ?>
	...
	<?php foreach ($list as $language) : ?>
		<option 
			data-content="
			<img style='display: inline;' title='<?php echo $language->title_native;?> ' alt='<?php echo $language->title_native;?> ' src='<?php echo JURI::base()?>media/mod_languages/images/<?php echo $language->image; ?>.png'>
			<span style='display: inline-block; padding: 0 5px;'><?php echo $language->title_native;?></span>" 
	...
	<div class="btn-group">
		<?php foreach ($list as $language) : ?>
			<?php if ($language->active) : ?>
				<?php $flag = ''; ?>
				<?php $flag .= "&nbsp;" . JHtml::_('image', 'mod_languages/' . $language->image . '.png', $language->title_native, array('title' => $language->title_native), true); ?>
