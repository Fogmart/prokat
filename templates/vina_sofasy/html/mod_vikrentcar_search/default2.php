...
function vrcSetLocOpenTimeModule(loc, where) {
	...
	jQuery.ajax({
		...
		if (where == "pickup" && vrcmod_location_change === false) {
			...
			$vrloc .= "<div class=\"vrcsfentrycont\"><label for=\"modplace\" class=\"modplacetitle\">".JText::_('VRMPPLACE')."</label><div class=\"vrcsfentryselect\"><select name=\"place\" id=\"modplace\"".$onchangeplaces.">";
			foreach ($places as $pla) {
				$vrloc .= "<option value=\"".$pla['id']."\"".(!empty($svrcplace) && $svrcplace == $pla['id'] ? " selected=\"selected\"" : "").">".$pla['name']."</option>\n";
			}
			$vrloc .= "</select></div></div>\n";
...
jQuery(function() {
	...
	jQuery('.vr-cal-img, .vrc-caltrigger').click(function() {
		...
		echo "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrylabsel\"><div class=\"vrcsfentrydate\"><input type=\"text\" name=\"pickupdate\" placeholder=\"Выберите дату\" required id=\"pickupdatemod".$randid."\" size=\"10\" autocomplete=\"off\"/><i class=\"fa fa-calendar vrc-caltrigger\"></i></div></div><div class=\"vrcsfentrytime\"><span id=\"vrcmodselph\"><select name=\"pickuph\" >" . $hours . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrcmodselpm\"><select name=\"pickupm\">" . $minutes . "</select></span></div></div>\n";
    ...
    if (@is_array($places)) {
    	$vrlocreturn = "";
    	$vrlocreturn .= "<div class=\"vrcsfentrycont\"><label for=\"modreturnplace\" class=\"modplacetitle\">".JText::_('VRMPLACERET')."</label><div class=\"vrcsfentryselect\"><select name=\"returnplace\" id=\"modreturnplace\"".(strlen($onchangeplacesdrop) > 0 ? $onchangeplacesdrop : "").">";
		foreach ($places as $pla) {
    ...
    //end mod carlos
		...
		echo "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrylabsel\"><div class=\"vrcsfentrydate\"><input type=\"text\" name=\"releasedate\" placeholder=\"Выберите дату\" required id=\"releasedatemod".$randid."\" size=\"10\" autocomplete=\"off\"/><i class=\"fa fa-calendar vrc-caltrigger\"></i></div></div><div class=\"vrcsfentrytime\"><span id=\"vrcmodseldh\"><select name=\"releaseh\">" . $hoursret . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrcmodseldm\"><select name=\"releasem\">" . $minutesret . "</select></span></div></div>";
	} else {
