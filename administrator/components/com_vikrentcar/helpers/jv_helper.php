...
if (!class_exists('VrcApplication')) {
	class VrcApplication {
		...
		{
			if (file_exists(VRC_ADMIN_PATH . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . $this->cms.'_'.$this->jv.'.css')) {
				JFactory::getDocument()->addStyleSheet(VRC_ADMIN_URI . 'helpers/' . $this->cms.'_'.$this->jv.'.css');
			}
		}
		...
		*/
		...
		public function getJmodalScript ($suffix = '', $hide_js = '', $show_js = '')
		{
			...
			if (!empty($hide_js)) {
				$hide_ev = "\n\t\t\t";
				$hide_ev .= "jQuery('.vrc-jmodal').on('$hide_evname', function() {
				".$hide_js."
			});";
				$hide_ev .= "\n";
			}
		...
		function vrcOpenJModal$suffix(id, modal_url, new_title) {
			if (modal_url) {
				...
				setTimeout(vrcFocusModal.bind(id), 500);
			}
			if (new_title) {
		...
		jQuery(document).ready(function() {
			jQuery('.vrc-jmodal').on('$show_evname', function() {
				$onshow_js
				$show_js
			});$hide_ev
		});
		...
		public function getJmodalHtml ($id, $title, $body = '', $style = '')
		{
			if ($this->jv < 4) {
				...
				return "\t\t".'<div class="modal vrc-jmodal hide fade" id="jmodal-'.$id.'" style="'.$style.'">
				...
				return "\t\t".'<div class="joomla-modal vrc-jmodal modal fade" id="jmodal-'.$id.'" role="dialog" tabindex="-1" style="'.$style.'">
		 ...
		 * @since 	1.12
		 */
		public function attachPopover($selector = '.vrcPopover', $options = array())
		{
			if (defined('JVERSION') && version_compare(JVERSION, '2.6.0') < 0) {
				// 2.5
		 ...
		 * @param 	array 	$options    An array of options for the popover
		 *
		 * @see 	VrcApplication::attachPopover() for further details about options keys.
		 *
		 * @since 	1.12
		 */
		public function createPopover($options = array())
		{
			...
			$options['icon_class'] = isset($options['icon_class']) ? $options['icon_class'] : 'vrcicn-question';

			// attach an empty array option so that the data will be recovered 
			// directly from the tag during the runtime
			$this->attachPopover(".vrc-quest-popover", array());

			if (defined('JVERSION') && version_compare(JVERSION, '2.6.0') < 0) {
				// 2.5
				return '<i class="'.$options['icon_class'].' vrc-quest-popover" title="'.$options['content'].'"></i>';
			} else {
				// 3.x
				$attr = '';
				...
				return '<i class="'.$options['icon_class'].' vrc-quest-popover" '.$attr.'></i>';
			}
		}
