...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="table table-striped">
	...
	for ($i = 0, $n = count($rows); $i < $n; $i++) {
		...
		<tr class="row<?php echo $k; ?>">
			...
			<td class="center"><?php echo (intval($row['perday'])==1 ? "<i class=\"fa fa-check vrc-icn-img\" style=\"color: #099909;\"></i>" : "<i class=\"fa fa-times-circle vrc-icn-img\" style=\"color: #ff0000;\"></i>"); ?></td>
			<td class="center"><?php echo (intval($row['hmany'])==1 ? "&gt; 1" : "1"); ?></td>
			<td><?php echo (file_exists(VRC_ADMIN_PATH.DS.'resources'.DS.$row['img']) && !empty($row['img']) ? "<span>".$row['img']." &nbsp;&nbsp;<img align=\"middle\" class=\"maxfifty\" src=\"".VRC_ADMIN_URI."resources/".$row['img']."\"/></span>" : ""); ?></td>
			<td class="center">
				<a href="index.php?option=com_vikrentcar&amp;task=sortoptional&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=up"><i class="fa fa-arrow-up vrc-icn-img"></i></a> 
				<a href="index.php?option=com_vikrentcar&amp;task=sortoptional&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=down"><i class="fa fa-arrow-down vrc-icn-img"></i></a>
			</td>
		</tr>
		<?php
		$k = 1 - $k;
