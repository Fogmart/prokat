...
$fprice = "<div class=\"vrc-fares-tabs\"><div class=\"dailypricesactive\">".JText::_('VRCDAILYFARES')."</div><div class=\"hourscharges\"><a href=\"index.php?option=com_vikrentcar&task=hourscharges&cid[]=".$idcar."\">".JText::_('VRCHOURSCHARGES')."</a></div><div class=\"hourlyprices\"><a href=\"index.php?option=com_vikrentcar&task=tariffshours&cid[]=".$idcar."\">".JText::_('VRCHOURLYFARES')."</a></div></div>\n";
if (empty($prices)) {
	$fprice .= "<br/><span class=\"err\"><b>".JText::_('VRMSGONE')." <a href=\"index.php?option=com_vikrentcar&task=newprice\">".JText::_('VRHERE')."</a></b></span>";
} else {
	$colsp = "2";
	$fprice .= "<form name=\"newd\" class=\"vrc-fares-frm\" method=\"post\" action=\"index.php?option=com_vikrentcar\" onsubmit=\"javascript: if (!document.newd.ddaysfrom.value.match(/\S/)){alert('".JText::_('VRMSGTWO')."'); return false;} else {return true;}\">\n<div class=\"vrc-insertrates-cont\"><span class=\"vrc-ratestable-lbl\">".JText::_('VRDAYS').": </span><table><tr><td><span class=\"vrc-fares-from-lbl\">".JText::_('VRDAYSFROM')."</span> <input type=\"number\" name=\"ddaysfrom\" id=\"ddaysfrom\" value=\"".(!is_array($prices) ? '1' : '')."\" min=\"1\" /></td><td>&nbsp;&nbsp;&nbsp; <span class=\"vrc-fares-to-lbl\">".JText::_('VRDAYSTO')."</span> <input type=\"number\" name=\"ddaysto\" id=\"ddaysto\" value=\"".(!is_array($prices) ? '30' : '')."\" min=\"1\" max=\"999\" /></td></tr></table>\n";
	$fprice .= "<span class=\"vrc-ratestable-lbl\">".JText::_('VRDAILYPRICES').": </span><table>\n";
	...
	$fprice .= "<tr><td colspan=\"".$colsp."\" align=\"right\"><input type=\"submit\" class=\"vrsubmitfares btn btn-large btn-success\" name=\"newdispcost\" value=\"".JText::_('VRINSERT')."\"/></td></tr></table></div><input type=\"hidden\" name=\"cid[]\" value=\"".$idcar."\"/><input type=\"hidden\" name=\"task\" value=\"tariffs\"/></form>";
}
...
$chcarsel .= "</select>\n";
$chcarf = "<form name=\"vrchcar\" method=\"post\" action=\"index.php?option=com_vikrentcar\"><input type=\"hidden\" name=\"task\" value=\"tariffs\"/>".$chcarsel."</form>";
