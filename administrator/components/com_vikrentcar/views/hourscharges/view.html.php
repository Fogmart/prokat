			...
			if (empty($phhoursto) || $phhoursfrom == $phhoursto) {
				foreach ($prices as $pr) {
					...
					if (!empty($tmpvarone)) {
						...
						$dbo->execute();
						if ($dbo->getNumRows() == 0) {
							$q = "INSERT INTO `#__vikrentcar_hourscharges` (`idcar`,`ehours`,`idprice`,`cost`,`attrdata`) VALUES('".$carrows['id']."',".$dbo->quote($phhoursfrom).",'".$pr['id']."','".($tmpvarone * $phhoursfrom)."',".($multipattr ? "'".($tmpvartwo  * $phhoursfrom)."'" : $dbo->quote($tmpvartwo)).");";
							$dbo->setQuery($q);
							$dbo->execute();
						} elseif ($dbo->getNumRows() == 1) {
							$upd_id = $dbo->loadResult();
							$q = "UPDATE `#__vikrentcar_hourscharges` SET `cost`='".($tmpvarone * $phhoursfrom)."', `attrdata`=".($multipattr ? "'".($tmpvartwo  * $phhoursfrom)."'" : $dbo->quote($tmpvartwo))." WHERE `id`=".(int)$upd_id." AND `ehours`=".$dbo->quote($phhoursfrom)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."' LIMIT 1;";
							$dbo->setQuery($q);
							$dbo->execute();
						}
					}
				...
				for ($i = intval($phhoursfrom); $i <= intval($phhoursto); $i++) {
					foreach ($prices as $pr) {
						...
						if (!empty($tmpvarone)) {
							...
							$dbo->execute();
							if ($dbo->getNumRows() == 0) {
								$q = "INSERT INTO `#__vikrentcar_hourscharges` (`idcar`,`ehours`,`idprice`,`cost`,`attrdata`) VALUES('".$carrows['id']."',".$dbo->quote($i).",'".$pr['id']."','".($tmpvarone * $i)."',".($multipattr ? "'".($tmpvartwo  * $i)."'" : $dbo->quote($tmpvartwo)).");";
								$dbo->setQuery($q);
								$dbo->execute();
							} elseif ($dbo->getNumRows() == 1) {
								$upd_id = $dbo->loadResult();
								$q = "UPDATE `#__vikrentcar_hourscharges` SET `cost`='".($tmpvarone * $i)."', `attrdata`=".($multipattr ? "'".($tmpvartwo  * $i)."'" : $dbo->quote($tmpvartwo))." WHERE `id`=".(int)$upd_id." AND `ehours`=".$dbo->quote($i)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."' LIMIT 1;";
								$dbo->setQuery($q);
								$dbo->execute();
							}
						}
